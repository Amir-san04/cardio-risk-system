  ...
  return (
    <div style={{ fontFamily:FONT, maxWidth:'960px', margin:'0 auto' }}>
      ...
      <div style={{ marginBottom:'1.5rem' }}>
        <h1 style={{ fontSize:'1.75rem', fontWeight:700, color:'#0f172a', margin:0 }}>Мой профиль</h1>
        <p style={{ color:'#64748b', margin:'0.25rem 0 0', fontSize:'0.9rem' }}>
          Ваши медицинские данные и история обследований
        </p>
      </div>
      ...
      <div style={{ background:'white', borderRadius:'16px', overflow:'hidden',
        ...
        <div style={{ background:'linear-gradient(135deg,#0f4c81,#0d7f8f)', padding:'2rem' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            ...
            {lastRisk && (
              <div style={{ background:'rgba(255,255,255,0.15)', borderRadius:'12px',
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', borderTop:'1px solid #f1f5f9' }}>
          {[
            ...
            <div key={i} style={{ padding:'1.25rem', borderRight: i < 3 ? '1px solid #f1f5f9' : 'none' }}>
              <div style={{ fontSize:'0.75rem', color:'#94a3b8', marginBottom:'0.25rem' }}>
                {item.icon} {item.label}
              </div>
              <div style={{ fontSize:'0.9rem', fontWeight:600, color:'#0f172a' }}>{item.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background:'white', borderRadius:'16px',
        ...
        {patient.examinations?.length === 0 ? (
          <div style={{ padding:'3rem', textAlign:'center', color:'#94a3b8' }}>
            У вас ещё нет обследований
          </div>
        ) : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ background:'#f8fafc' }}>
                {['Дата','Тип','Жалобы','Риск'].map(h => (
